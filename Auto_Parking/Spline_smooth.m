function[x,y,theta] = Spline_smooth(PX,PY)

global Lmin Amin kmax vehicle_kinematics_; 

kmax = vehicle_kinematics_.vehicle_kappa_max;
% Amin = pi - vehicle_kinematics_.vehicle_v_max * vehicle_kinematics_.vehicle_kappa_max;
Amin = pi - 0.8;%Amin = pi-theta_max = pi-v_max*kappa_max
Lmin = 1/6*sin(Amin)*1/kmax*((1-cos(Amin)/8))^(-3/2);
% Lmin = 15;
theta = [];
...
function[x,y] = BoundarySpline(x0,y0,t0,xf,yf,tf)
    ...
    global  Lmin;
    global Amin;
    thro = 3.3243; %kapa = 1/thro
%     Amin = pi - 0.7;% phy_max=0.7, so, Amin=pi-phy_max;
%     Amin =pi/1.2; 
%     Lmin = 10;
...
function [px,py]=sharp(px,py)%改0629
    global  Lmin;
    global  Amin;
    ...
    while i <((size(px,2)-3)/2)
        ...
        if gamma<(Amin)
            ...
            px1=px(n+2)+Lmin*cos(Amin+ang);
            py1=py(n+2)+Lmin*sin(Amin+ang);
            d1 = sqrt((px1-px(n+6))^2+(py1-py(n+6))^2);
            px2=px(n+2)-Lmin*cos(Amin+ang);
            py2=py(n+2)-Lmin*sin(Amin+ang);
...
u=0:1/999:1; %parameter:draw999,default99
x=u*0;
y=u*0;
...
for deg=3
    ...
    % figure(1); plot(x,y,'LineWidth',1, 'Color',[34,147,216]/255);grid off; xlabel ('x','FontName','Times New Roman','FontAngle','Italic','FontSize',22); ylabel('y','FontName','Times New Roman','FontAngle','Italic','FontSize',22);hold on;
