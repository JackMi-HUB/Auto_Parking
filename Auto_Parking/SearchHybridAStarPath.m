function [x, y, theta, path_length, completeness_flag] = SearchHybridAStarPath(start_config,end_config,tc)%tc为停止时间
global hybrid_astar_ vehicle_kinematics_
grid_space_ = cell(hybrid_astar_.num_nodes_x, hybrid_astar_.num_nodes_y, hybrid_astar_.num_nodes_theta);
% end_config = [vehicle_TPBV_.xtf, vehicle_TPBV_.ytf, vehicle_TPBV_.thetatf];%global_pose
% start_config = [vehicle_TPBV_.x0, vehicle_TPBV_.y0, vehicle_TPBV_.theta0];%global_pose
...
init_node(6) = CalculateH(start_config, end_config,tc);
init_node(5) = 0;
...
iter = 0;
tic
while ((~isempty(openlist_)) && (iter <= hybrid_astar_.max_iter) && (~completeness_flag) && (toc <= tc))%initial:15.0
    iter = iter + 1;
    ...
    for ii = 1 : 6
        ...
        child_node(6) = CalculateH(child_node_config, end_config,tc);
        ...
        N = 0.8 * distance - 3;%如何决定N？？
        counter = counter + 1;
        % Specify analytical RS curve generator is activated.
        if (counter > N)
            counter = 0;
            [x_rs, y_rs, theta_rs, path_length] = GenerateRsPath(child_node_config, end_config);%child_node是目前为止最佳节点
...
indx = round((xx - environment_scale_.environment_x_min) /  hybrid_astar_.resolution_x + 1);% +1) or )+1？？？
...
function val = CalculateH(start_config, end_config,tc)
...
distance_holonomic_with_collision_avoidance = CalculateAStarPathLength(start_config, end_config,tc);
...
reedsConnObj = reedsSheppConnection('MinTurningRadius', vehicle_kinematics_.vehicle_turning_radius_min);
...
function path_length = CalculateAStarPathLength(start_config, end_config,tc)
...
while ((~isempty(openlist_))&&(iter <= hybrid_astar_.num_nodes_x^2)&& (toc <= tc))
    iter = iter + 1;
